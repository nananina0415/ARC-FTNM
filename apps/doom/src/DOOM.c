...
static unsigned char* final_screen_buffer[2] = { 0, 0 };
static int back_buf_idx = 0;
...
void doom_init(int argc, char** argv, int flags)
{
    ...
    final_screen_buffer[0] = doom_malloc(DOOM_OUT_WIDTH * DOOM_OUT_HEIGHT * 4);
    final_screen_buffer[1] = doom_malloc(DOOM_OUT_WIDTH * DOOM_OUT_HEIGHT * 4);
...
const unsigned char* doom_get_framebuffer(int channels)
{
    ...
    /* 백버퍼 선택 후 인덱스 교체 (다음 호출은 반대 버퍼에 씀) */
    unsigned char* buf = final_screen_buffer[back_buf_idx];
    back_buf_idx ^= 1;
    ...
    {
        /* 320×200 → 640×480 스케일업 (수평 2×, 수직 1.2×)
         * oy * SCREENHEIGHT / DOOM_OUT_HEIGHT 로 입력 행 결정.
         * 같은 입력 행이 반복되는 출력 행은 이전 출력 행을 memcpy — 팔레트
         * 변환과 수평 더블링은 입력 행당 한 번만 수행한다. */
        int oy, prev_in_y = -1;
        for (oy = 0; oy < DOOM_OUT_HEIGHT; oy++)
        {
            int in_y = oy * SCREENHEIGHT / DOOM_OUT_HEIGHT;
            unsigned char* dst = buf + oy * DOOM_OUT_WIDTH * 3;
            if (in_y == prev_in_y)
            {
                doom_memcpy(dst, dst - DOOM_OUT_WIDTH * 3, DOOM_OUT_WIDTH * 3);
            }
            else
            {
                const unsigned char* src = screen_buffer + in_y * SCREENWIDTH;
                for (sx = 0; sx < SCREENWIDTH; sx++)
                {
                    int kpal = src[sx] * 3;
                    unsigned char r = screen_palette[kpal + 0];
                    unsigned char g = screen_palette[kpal + 1];
                    unsigned char b = screen_palette[kpal + 2];
                    unsigned char* p = dst + sx * 6;
                    p[0]=r; p[1]=g; p[2]=b;
                    p[3]=r; p[4]=g; p[5]=b;
                }
                prev_in_y = in_y;
    ...
    else if (channels == 4)
    {
        ...
        {
            int in_y = oy * SCREENHEIGHT / DOOM_OUT_HEIGHT;
            unsigned char* dst = buf + oy * DOOM_OUT_WIDTH * 4;
            if (in_y == prev_in_y)
            {
                doom_memcpy(dst, dst - DOOM_OUT_WIDTH * 4, DOOM_OUT_WIDTH * 4);
            }
            else
            {
                const unsigned char* src = screen_buffer + in_y * SCREENWIDTH;
                for (sx = 0; sx < SCREENWIDTH; sx++)
                {
                    ...
                    unsigned char g = screen_palette[kpal + 1];
                    unsigned char b = screen_palette[kpal + 2];
                    unsigned char* p = dst + sx * 8;
                    p[0]=r; p[1]=g; p[2]=b; p[3]=255;
                    p[4]=r; p[5]=g; p[6]=b; p[7]=255;
                }
                prev_in_y = in_y;
            }
        }
