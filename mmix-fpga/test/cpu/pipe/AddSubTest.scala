...
/** AddSub를 감싸서 레지스터버스 쪽 신호(ack/data)를 평평한 IO로 노출하는 테스트용 하네스.
 * RegArbiter가 아직 목업이라 실제 버스 대신 여기서 직접 ack/data를 준다.
 */
class AddSubHarness extends Module {
  val io = IO(new Bundle {
    ...
    val result    = Output(AddSubResult())
  })

  val arbiter = Module(new RegArbiter)
  val factory = new RegReadPortFactory(arbiter)
  val addsub  = Module(new AddSub(factory))

  addsub.io.op         := io.op
  addsub.io.pause      := io.pause
  addsub.io.reg.x.data := 0.U   // AddSub는 X(목적지)를 읽지 않음 — req가 항상 0이라 값은 안 쓰임
  addsub.io.reg.x.ack  := false.B
  addsub.io.reg.y.data := io.regYData
  addsub.io.reg.y.ack  := io.regYAck
  addsub.io.reg.z.data := io.regZData
  addsub.io.reg.z.ack  := io.regZAck
  io.result := addsub.io.result
}
...
class AddSubTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  // 파이프 입력 설정 헬퍼
  def setOp(dut: AddSubHarness, f: Int, x: Int, y: Int, z: Int): Unit = {
   ...
   * 즉치 모드(Y든 Z든)에선 해당 ack/data는 무시되니 아무 값이나 넘겨도 무해하다.
   */
  def issue(dut: AddSubHarness, f: Int, x: Int, y: Int, z: Int, yData: BigInt, zData: BigInt): Unit = {
    dut.io.regYAck.poke(false.B)
    dut.io.regZAck.poke(false.B)
    setOp(dut, f, x, y, z)
    dut.clock.step()

    dut.io.regYData.poke(yData.U(64.W))
    dut.io.regZData.poke(zData.U(64.W))
  ...
  "AddSub" should "ADD: 3 + 5 = 8" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x00, 1, 2, 3, 3, 5)  // ADD|I64, x=$1, y=$2, z=$3
  ...
  "AddSub" should "ADD: 오버플로우 감지 (max + 1)" in {
    test(new AddSubHarness) { dut =>
      val maxI64 = BigInt("7fffffffffffffff", 16)
      issue(dut, 0x00, 0, 1, 2, maxI64, 1)
  ...
  "AddSub" should "ADDU: 오버플로우 트랩 없음" in {
    test(new AddSubHarness) { dut =>
      val maxU64 = BigInt("ffffffffffffffff", 16)
      issue(dut, 0x02, 0, 1, 2, maxU64, 1)  // ADD|U64
  ...
  "AddSub" should "ADDUI: 10 + imm(5) = 15, 트랩 없음" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x03, 0, 1, 5, 10, 0)  // ADD|U64|IMM
  ...
  "AddSub" should "SUB: 10 - 3 = 7" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x04, 0, 1, 2, 10, 3)  // SUB|I64
  ...
  "AddSub" should "SUB: MIN - 1 → 오버플로우 (부호 다른 두 수, 결과 부호가 Y와 달라짐)" in {
    test(new AddSubHarness) { dut =>
      val minI64 = BigInt("8000000000000000", 16)
      val maxI64 = BigInt("7fffffffffffffff", 16)
      issue(dut, 0x04, 0, 1, 2, minI64, 1)
      dut.io.result.res.expect(maxI64.U(64.W))
      dut.io.result.ovf.expect(true.B)
  ...
  "AddSub" should "SUBI: 10 - imm(3) = 7" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x05, 0, 1, 3, 10, 0)  // SUB|I64|IMM
      dut.io.result.res.expect(7.U)
    }
  }
  ...
  "AddSub" should "SUBU: MIN - 1 → 결과는 SUB와 동일하지만 트랩 없음" in {
    test(new AddSubHarness) { dut =>
      val minI64 = BigInt("8000000000000000", 16)
      val maxI64 = BigInt("7fffffffffffffff", 16)
      issue(dut, 0x06, 0, 1, 2, minI64, 1)  // SUB|U64
      dut.io.result.res.expect(maxI64.U(64.W))
      dut.io.result.ovf_trap.expect(false.B)
    }
  }
  ...
  "AddSub" should "SUBUI: 10 - imm(3) = 7, 트랩 없음" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x07, 0, 1, 3, 10, 0)  // SUB|U64|IMM
      dut.io.result.res.expect(7.U)
      dut.io.result.ovf_trap.expect(false.B)
    }
  }
  ...
  "AddSub" should "NEG: 5 - $Z(3) = 2" in {
    test(new AddSubHarness) { dut =>
      ...
      issue(dut, 0x08, 0, 5, 3, 0, 3)  // x=$0, y=5(즉치), z=$3
  ...
  "AddSub" should "NEG: Y=0, $Z=MIN → 오버플로우 (문서에 명시된 특수 조건)" in {
    test(new AddSubHarness) { dut =>
      val minI64 = BigInt("8000000000000000", 16)
      issue(dut, 0x08, 0, 0, 3, 0, minI64)  // NEG|I64, Y=0
      dut.io.result.res.expect(minI64.U(64.W))
      dut.io.result.ovf.expect(true.B)
      dut.io.result.ovf_trap.expect(true.B)
    }
  }
  ...
  "AddSub" should "NEGI: 5 - imm(3) = 2" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x09, 0, 5, 3, 0, 0)  // NEG|I64|IMM
      dut.io.result.res.expect(2.U)
    }
  }
  ...
  "AddSub" should "NEGU: Y=0, $Z=MIN → 결과는 NEG와 동일하지만 트랩 없음" in {
    test(new AddSubHarness) { dut =>
      val minI64 = BigInt("8000000000000000", 16)
      issue(dut, 0x0A, 0, 0, 3, 0, minI64)  // NEG|U64
      dut.io.result.res.expect(minI64.U(64.W))
      dut.io.result.ovf_trap.expect(false.B)
    }
  }
  ...
  "AddSub" should "NEGUI: 5 - imm(3) = 2, 트랩 없음" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x0B, 0, 5, 3, 0, 0)  // NEG|U64|IMM
      dut.io.result.res.expect(2.U)
      dut.io.result.ovf_trap.expect(false.B)
    }
  }
  ...
  "AddSub" should "2ADDU: 2*4 + 3 = 11" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x12, 0, 1, 2, 4, 3)  // SHIFT2ADDU bits[4:0] = 10010
  ...
  "AddSub" should "ADDI: $Y + imm(10) = 15" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x01, 0, 1, 10, 5, 0)  // ADD|I64|IMM, z=10(즉치)
      dut.io.result.res.expect(15.U)
    }
  }
  ...
  "AddSub" should "pause=1이면 이전 값 유지" in {
    test(new AddSubHarness) { dut =>
      issue(dut, 0x00, 0, 1, 2, 7, 3)  // 7+3=10 래치
      dut.io.result.res.expect(10.U)

      // pause=1, 입력 바꿔도 버퍼 유지
      dut.io.pause.poke(true.B)
      dut.io.regYData.poke(100.U)
      dut.io.regZData.poke(200.U)
      dut.clock.step()
